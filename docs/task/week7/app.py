...
def memberApi():
    ...
    if username:
        db_connect = Website('localhost', 'root', 'root')
        user = db_connect.queryMemberByUserName(username)
        print(type(user))
        if user:
            data = {
                'id' : user['id'],
...
def updateMember():
    ...
    orig_username = member['username']
    input_date = request.get_json()
    
    if input_date and orig_username:
        db_connect = Website('localhost', 'root', 'root')
        db_connect.updateMemberUserName(orig_username, input_date['name'])
